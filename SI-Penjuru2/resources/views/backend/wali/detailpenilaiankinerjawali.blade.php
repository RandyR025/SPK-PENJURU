...
@php $questionNum = 1; @endphp
    ...
    <div class="card mb-3">
                ...
                <div class="sw-5 me-4 d-flex justify-content-center flex-grow-0 flex-shrink-0">
                    <div class="d-flex justify-content-center align-items-center">
                        <label class="btn btn-foreground sw-4 sh-4 p-0 rounded-xl stretched-link" for="mc_c{{ $choicenum }}">
                            ...
                            <input type="radio" class="option form-check-input" id="mc_c{{ $choicenum++ }}" name="answer[{{ $questionNum }}]" value="{{ $p->kode_pilihan}}" onclick="handleClickWali(this,'<?=$p->kode_pengisian;?>','<?=$user[0]->id;?>');" <?php if(hasilPilihanWali($p['kode_pilihan'],$user[0]->id, Auth::user()->id)) echo 'checked'?>>
                        </label>
                    </div>
                </div>
        ...
        <div class="col-12 text-center">
            <a href="{{ route('gettotalnilaiwali', [$item->id_penilaian,$user[0]->id])}}">
